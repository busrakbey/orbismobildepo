...
/**
 * Created by isahin on 10.7.2017.
 */
...
public class MBTilesLayer extends TiledServiceLayer {
    ...
    public MBTilesLayer(String path) {
        super(path);
        try {
            mapDb = SQLiteDatabase.openDatabase(path, null, SQLiteDatabase.NO_LOCALIZED_COLLATORS);
        } catch (SQLException ex) {

            Log.v("tile path","=>"+path+"**"+mapDb.getPath());
    ...
    protected byte[] getTile(int level, int col, int row) throws Exception {
        ...
        try {
            imageCur = mapDb.rawQuery("SELECT tile_data FROM tiles WHERE zoom_level = " + Integer.toString(level)
